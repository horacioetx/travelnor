<?php
	...
	$page_id = $_REQUEST['pagid'];
	
	/* retrieve info from table */
	
	$stmt = $db->prepare('SELECT * FROM dir_web_pages WHERE page_id = :page_id');	
	...
	if ($rrows['page_status'] == 0)	
		$status = '<span class="text-success">Activa</span>';
	else
		$status = '<span class="text-danger">Inactiva</span>';
	
    /* page title */
	
	if (isset($conrow['company_name']))	
        $title = $conrow['company_name'] . ' - ' . str_replace("<br />", " ", $rrows['page_name']);
    else	
        $title = str_replace("<br />", " ", $rrows['page_name']);
			...
			<div class="dash-app">
				...
				<nav class="bg-light" aria-label="breadcrumb">
					<ol class="breadcrumb">								
						...
						<li class="breadcrumb-item"><a href="dir_pages">Directorio de Páginas web</a></li>
						<li class="breadcrumb-item active" aria-current="page"><?php echo $rrows['page_name']; ?></li>						
					</ol>
				</nav>					
					...
					<div class="container-fluid">
						...
						<div class="row">						
                            <div class="col">	
								<div class="w-100 align-items-left pb-2 mb-1">
									<a href="https://<?php echo $conrow['company_website'] . '/' . $rrows['page_name']; ?>" target="_blank" style="color:inherit;"><button type="button" class="btn btn-info btn-sm mr-2 mt-2 text-white float-right"><i class="fas fa-search mr-2"></i>Vista Previa</button></a>
								</div>
							</div>
						</div>
						...
						<div class="jumbotron jumbotron-fluid mt-4 py-2 border-top border-bottom bg-transparent">
							<div class="container ml-0 pl-0">
								<h2 class="text-dark">Página Web : <span class="text-success"><?php echo str_replace("<br />", " ", $rrows['page_name']); ?></span></h2>
							</div>
						</div>
						...
						<div class="row">						
							<div class="col-12">							
                                ...
                                <div id="general_info"><?php include 'dir_pages_view_general_disp.php'; ?> </div>								
						...
						<div class="jumbotron jumbotron-fluid mt-5 py-2 border-top border-bottom bg-transparent">
							<div class="container ml-0 pl-0">
								<h5 class mb-3><strong>Contenido</strong><button type="button" class="btn btn-success btn-sm float-right" data-toggle="modal" data-target="#newsubcontact">Agregar Contenido</button></h5>
							</div>
						</div>
                        ...
                        <div class="row mt-5" id="content">   
							<div class="col-12">		
								...
								<?php include 'dir_pages_view_content.php'; ?>
